...
# Face = namedtuple("Face", [ScutoidFace, restraint])
...
class Point_2d:
    ...
    def __sub__(self, other):
        return Point_2d(self.x - othher.x, self.y - other.y)
...
#    linear_coefficient_x = -2.0 * point.x
#    linear_coefficient_y = -2.0 * point.y
#    linear_coefficient_z = -2.0 * point.z
...
#  ?
def is_squeezed_out(plane, seed, left, center, right):
    # if there was no center neighbour seed voronooi cell the space would
    # be filled up by neighbouring cells. if the meeting points on a plane
    # is further away form the center cell seed than the other cell seeds
    # this point is real and the face where the seed cell and the cennter meet
    # is probably a triangle on the other side.
    
    squeeze_point = restraint_solver(plane, seed, left, right)
    return dist(seed, squeeze_point) < dist(seed, center)  # continue here
...
# can do without?
def test_is_squeezed_out():
    print("@@@ test_is_squeezed_out @@@")
    print(is_squeezed_out(Plane(0.0, 0.0, 1.0, 1.0),
                          Point(0.0, 0.0, 0.0),
                          Point(0.0, 1.0, 0.0),
                          Point(0.6, 0.6, 0.1),
                          Point(1.0, 0.0, 0.0)
...
def scutoid1():
    ...
    a_point = restraint_solver(center, upper_plane, opposite, neighbour_1)
    b_point = restraint_solver(center, upper_plane, neighbour_1, neighbour_2)
    c_point = restraint_solver(center, upper_plane, neighbour_2, neighbour_3)
    d_point = restraint_solver(center, upper_plane, nieghbour_3, triangle_neighbour)
    e_point = restraint_solver(center, upper_plane, triangle_neighbour, neighbour_4)
    f_point = restraint_solver(center, upper_plane, neighbour_4, opposite)
    g_point = restraint_solver(center, triangle_neighbour, neighbour_3, nieghbour_4)
    h_point = restraint_solver(center, lower_plane, opposite, neighbour_1)
    i_point = restraint_solver(center, lower_plane, neighbour_1, neighbour_2)
    j_point = restraint_solver(center, lower_plane, neighbour_2, neighbour_3)
    k_point = restraint_solver(center, lower_plane, neighbour_3, neighbour_4)
    l_point = restraint_solver(center, lower_plane, neighbour_4, opposite)

    lkgef_face = [l_point, k_point, g_point, e_point, f_point]
    # alignment of the first face
    distance = dist(l_point, f_point)
    anchorpoint_2 = np.array([distance * POLYGON_SCALE * math.cos(POLYGON_START_ANGLE),
                              distance * POLYGON_SCALE * math.sin(POLYGON_START_ANGLE)]) + POLYGON_START
    # We state that the attach polygon function scales the polygon to attach to mach the length.
    lfgef_face_net = attach_polygon(POLYGON_START, anchorpoint_2, lfgef_face)
    draw_polygon(lfgef_face_plane)
    lfah_face = [l_point, f_point, a_point, h_point]
    flah_face_net = attach_polygon(lfgef_face_net[1], lfgef_face_net[0], flah_face)
    draw_polygon(flah_face_net)
    habi_face = [h_point, a_point, b_point, i_face]
    habi_face_net = attach_polygon(flah_face_net[3], flah_face_net[2], habi_face)
    draw_polygon(habi_face_net)
    bijc_face = [b_point, i_point, j_point, c_point]
    bijc_face_net = attach_polygon()
